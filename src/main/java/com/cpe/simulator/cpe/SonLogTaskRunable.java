...
@Data
@Slf4j
public class SonLogTaskRunable implements Runnable {

    private String serialNumber = "902514400004";
    ...
    private String uploadUrl = "http://172.16.3.34:18088/api/httpfs/v1/upload/sonLog/902514400004/";

    @Override
    public void run() {
        try {
            String sourceFilePath = uploadFileDir + File.separator + "sonOptimize_0_log.csv";
            ...
            String targetFileName = "sonOptimize_0_log.csv";
            ...
            log.info("upload SonLog file result:" + response.getStatusCode().value());
            sendUploadSuccessMsg(length, targetFileName, uploadUrl);
        } catch (Exception e) {
            log.error(e.getMessage(), e);
        }
    }
    ...
    public boolean sendUploadSuccessMsg(long fileSize, String fileName, String url) {
        ...
        try {
            ...
            autonomousTransferComplete.setFileType(InformConstants.SON_LOG_FILE_TYPE);
